...
// GetPendingApprovals provides a mock function with given fields: ctx, groupID
func (_m *MemberRepository) GetPendingApprovals(ctx context.Context, groupID uuid.UUID) ([]domain.SentInvitationResponse, error) {
	ret := _m.Called(ctx, groupID)

	if len(ret) == 0 {
		panic("no return value specified for GetPendingApprovals")
	}
